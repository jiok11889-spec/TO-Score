...
def build_data():
    ...
    return {
        'summary':{'total_in':total_in,'total_out':total_out,'balance':balance,'avg3':avg_n(3),'avg6':avg_n(6),'avg12':avg_n(12),'updated':now.strftime('%Y-%m-%d %H:%M'),'latest_ym':latest_ym},
        ...
        'fc_base':{'month':fc_base_month,'year':fc_base_year},
...
:root{
  ...
  --p:#4B9EFF;
  --p-dim:rgba(75,158,255,0.10);
  --p-dim2:rgba(75,158,255,0.18);
  --w:#ffffff;
...
nav{position:sticky;top:0;z-index:50;background:rgba(8,8,8,0.8);backdrop-filter:blur(24px);border-bottom:1px solid var(--line);padding:0 28px;height:52px;display:flex;align-items:center;justify-content:space-between;}
...
.sc-pill.on{background:var(--p-dim2);color:var(--p);border-color:rgba(75,158,255,0.3);}
...
.tag-지각비{background:rgba(75,158,255,0.07);color:rgba(75,158,255,0.6);}
.tag-이자{background:rgba(255,255,255,0.05);color:var(--w3);}
.tag-기타{background:rgba(255,255,255,0.05);color:var(--w3);}
.empty-state{text-align:center;padding:40px 20px;color:var(--w3);font-size:12px;line-height:1.8;}
...
<body>
<nav>
  <span class="nav-logo"><em>T</em>O</span>
  <div class="nav-right">
...
<div class="page">
  ...
  <div class="sec">
    ...
    <div class="leg-row">
      <span class="leg"><span class="leg-line" style="background:#4B9EFF"></span>입금</span>
  ...
  <div class="two">
    <div class="sec" style="margin-bottom:0">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
        ...
        <div id="acct-box" onclick="copyAcct()" title="클릭하면 계좌번호 복사" style="cursor:pointer;background:rgba(75,158,255,0.08);border:1px solid rgba(75,158,255,0.2);border-radius:8px;padding:5px 11px;text-align:right;">
          <div id="acct-label" style="font-size:9px;color:rgba(75,158,255,0.6);letter-spacing:0.8px;margin-bottom:2px;">카카오뱅크</div>
          <div id="acct-num" style="font-size:12px;font-weight:500;color:#4B9EFF;letter-spacing:1px;">—</div>
        </div>
      </div>
...
function render(){
  ...
  document.getElementById(\'acct-label\').textContent=`카카오뱅크 · ${acct.holder}`;
  document.getElementById(\'acct-num\').textContent=acct.number;
  buildMain(); buildFc(); buildPaySelect(); buildMembers(); buildTL();
}

function copyAcct(){
  const num=D.account.number;
  if(num===\'계좌번호 미설정\') return;
  navigator.clipboard.writeText(num.replace(/-/g,\'\')).then(()=>{
    const el=document.getElementById(\'acct-num\');
...
function buildMain(){
  ...
  if(mode===\'cum\'){
    ...
    ds=[
      {type:mode,label:\'입금\',data:real.map(m=>m.income),backgroundColor:mode===\'bar\'?\'rgba(75,158,255,0.65)\':\' transparent\',borderColor:\'#4B9EFF\',borderRadius:mode===\'bar\'?3:0,borderWidth:1.5,tension:0.4,pointRadius:0,fill:false},
...
function iqrFilter(arr){
  ...
  const iqr=q3-q1; const upper=q3+1.5*iqr;
...
function buildFc(){
  ...
  const ai=li.length?Math.round(li.reduce((a,m)=>a+m.income,0)/li.length):820000;
  const lo=real.filter(m=>m.expense>0).slice(-14);
  const loVals=lo.map(m=>m.expense);
  const loFiltered=iqrFilter(loVals.length?loVals:[0]);
  const ao=loFiltered.length?Math.round(loFiltered.reduce((a,v)=>a+v,0)/loFiltered.length):0;
  const excluded=loVals.length-loFiltered.length;
  const cb=D.summary.balance;
  const base=D.fc_base;
  const scenarios=[
    {label:\'지출 -20%\', in:ai, out:Math.round(ao*0.8), color:\'rgba(75,158,255,1.0)\',  dim:\'rgba(75,158,255,0.07)\',  dash:[]},
    {label:\'지출 -10%\', in:ai, out:Math.round(ao*0.9), color:\'rgba(75,158,255,0.45)\', dim:\'rgba(75,158,255,0.03)\',  dash:[4,3]},
  ...
  const fcLabels=[];
  for(let i=1;i<=12;i++){
    const bm=base.month+i-1;
    const yy=base.year+Math.floor(bm/12);
    const mm=(bm%12)||12;
    fcLabels.push(`${yy%100}년 ${mm}월`);
  }
  const allL=[...hist.map(m=>m.month),...fcLabels];
  const histConnect=hist.length?hist[hist.length-1].cumulative:cb;
  const datasets=[
  ...
  scenarios.forEach(sc=>{
    ...
    datasets.push({label:sc.label,data:[...new Array(Math.max(hist.length-1,0)).fill(null),histConnect,...pts],
      borderColor:sc.color,backgroundColor:sc.dim,fill:false,tension:0.4,pointRadius:0,borderWidth:1.5,borderDash:sc.dash});
  });
  if(FC)FC.destroy();
  FC=new Chart(document.getElementById(\'fcC\'),{
    type:\'line\',data:{labels:allL,datasets},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{mode:\'index\',intersect:false,callbacks:{label:c=>c.raw!=null?` ${c.dataset.label}: ${fmtK(c.raw)}원`:null,filter:c=>c.raw!=null}}},
      scales:{
  ...
  if(desc) desc.innerHTML=`입금 기준: 최근 6개월 평균 <b style="color:rgba(255,255,255,0.7)">${fmtK(ai)}원</b> &nbsp;·&nbsp; 지출 기준: 최근 12개월 IQR 평균 <b style="color:rgba(255,255,255,0.7)">${fmtK(ao)}원</b> (비경상 ${excluded}개월 제외)`;
  const fcc=document.getElementById(\'fc-cards\');
  fcc.innerHTML=\'\';
  const cardStyles=[
    {bg:\'rgba(75,158,255,0.18)\',border:\'rgba(75,158,255,0.35)\',mo:\'rgba(75,158,255,0.55)\',val:\'#4B9EFF\',chg:\'rgba(75,158,255,0.7)\'},
    {bg:\'rgba(75,158,255,0.07)\',border:\'rgba(75,158,255,0.18)\',mo:\'rgba(75,158,255,0.4)\', val:\'rgba(75,158,255,0.8)\',chg:\'rgba(75,158,255,0.5)\'},
  ...
  scenarios.forEach((sc,i)=>{
    ...
    const chg=bal12-cb; const isP=chg>=0; const netMo=sc.in-sc.out; const cs=cardStyles[i];
...
function buildPaySelect(){
  ...
  sel.innerHTML=\'\';
  if(!D.pay_months.length){
    const opt=document.createElement(\'option\'); opt.textContent=\'데이터 없음\'; sel.appendChild(opt); return;
  }
...
function buildTL(){
  ...
  if(!D.recent.length){ t.innerHTML=\'<div class="empty-state">원장에 거래내역을 입력해 주세요.</div>\'; return; }
  D.recent.forEach(r=>{
    const cls=r.is_income?\'in\':\'out\', sign=r.is_income?\'+\':\'-\', icon=r.is_income?\'↑\':\'↓\';
...
if __name__=='__main__':
    ...
    if env_port:
        ...
        print(f'TO-Score 대시보드 실행 중: http://0.0.0.0:{port}')
