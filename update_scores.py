...
# 경계 타이(랭킹 7/8, 14/15, 21/22, 28/29위) 없음 → 순수 순위 배분, 규칙C 개입 없음
#
# 4차→5차 변경분 (기준스코어 재계산 결과, 아래 8명만 이동):
#   강충현  T4→T3  (기준점수 96.0, 랭킹 21위)
#   김지수  T3→T4  (기준점수 96.5, 랭킹 22위)
#   이호준  T5→T4  (기준점수 96.7, 랭킹 23위)
#   김현섭  T5→T4  (기준점수 97.6, 랭킹 24위)
#   최원재  T5→T4  (기준점수 98.8, 랭킹 26위)
#   이건희  T4→T5  (기준점수 99.8, 랭킹 29위)
#   황호연  T4→T5  (기준점수 100.3, 랭킹 30위)
#   이도영  T4→T5  (기준점수 101.2, 랭킹 31위)
TIER_MAP = {
