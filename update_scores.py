...
# 4차 대회(6/12) 이후 기준스코어(누적평균×0.5 + 26년평균×0.5) 기반 티어 재정렬
# T0 = PRO(config.py) + 기록없는 오지호 고정
# T1~T5 = 비PRO 기준스코어 오름차순 7/7/7/7/8명 배분 (기록없는 멤버는 현 티어 유지)
#
# 주요 변경:
#   나한영·홍경택  T0→T1  (기준점수 83.2, 랭킹 4·5위)
#   김대우        T1→T2  (기준점수 86.3, 랭킹 8위)
#   김성환        T1→T2  (기준점수 89.2, 랭킹 10위)
#   이준범        T3→T2  (기준점수 92.5, 랭킹 14위)
#   윤현호·류성준·조현민·김성환(92)·권기표  T2→T3  (랭킹 15~21위)
#   김지수        T4→T3  (기준점수 96.5, 랭킹 20위)
#   이건희·박헌우·황호연  T3→T4  (랭킹 22·23·28위)
#   손원우        T2→T4  (기준점수 99.0, 랭킹 26위)
#   이도영        T5→T4  (기준점수 99.1, 랭킹 27위)
#   김현섭·이호준·최원재·김서래  T4→T5  (랭킹 29·31·32·33위)
#   백근호        T3→T5  (기준점수 101.3, 랭킹 30위)
TIER_MAP = {
    # 티어 0: PRO 5명 + 오지호(기록없음 고정)
    "김산":        0, "이형석":    0, "이현우":     0, "윤석원":    0,
    "임승언":      0, "오지호":    0,
    # 티어 1 (11명): 기준스코어 랭킹 1~7위 + 기록없는 4명
    "이정승":      1, "최창환":    1, "이은광":     1, "나한영":    1,
    "홍경택":      1, "김수민":    1, "박병우":     1,
    "박계홍":      1, "박세환":    1, "신태범":     1, "노용현":    1,
    # 티어 2 (10명): 기준스코어 랭킹 8~14위 + 기록없는 3명
    "김대우":      2, "윤준원":    2, "김성환":     2, "강현수":    2,
    "임익준":      2, "황인호":    2, "이준범":     2,
    "이민재":      2, "권한해":    2, "고영승":     2,
    # 티어 3 (12명): 기준스코어 랭킹 15~21위 + 기록없는 5명
    "윤현호":      3, "류성준":    3, "조현민":     3, "김성환(92)": 3,
    "조성태":      3, "김지수":    3, "권기표":     3,
    "최성호":      3, "박준상":    3, "한상윤":     3, "최정훈":    3, "손정모":    3,
    # 티어 4 (11명): 기준스코어 랭킹 22~28위 + 기록없는 4명
    "이건희":      4, "박헌우":    4, "강충현":     4, "김준기":    4,
    "손원우":      4, "이도영":    4, "황호연":     4,
    "박상용":      4, "박문석":    4, "최창규":     4, "노민일":    4,
    # 티어 5 (14명): 기준스코어 랭킹 29~36위 + 기록없는 6명
    "김현섭":      5, "백근호":    5, "이호준":     5, "최원재":    5,
    "김서래":      5, "옥지엽":    5, "서정권":     5, "주홍석":    5,
    "신재민":      5, "김진호":    5, "강승원":     5, "이종화":    5,
    "박승현":      5, "강명신":    5,
}
